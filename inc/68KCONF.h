...
/// MASKING MACROS TO HELP WITH EXTRACTING
/// THE BITS NECESSARY FOR BIG-ENDIAN DATA OPERATIONS
/// WHEN USED, THE PARAMETER PASSED THROUGH WILL BE EXTRACTED
// ON THE BASIS OF THESE MAX VALUE MASKS FOR B,W AND L
...
/// RETURNS BIT SHIFT MASKS BASED ON THE FULL VALUE ENCOMPASSING
// EACH RESPECTIVE DATA OPERATION
...
#define		M68K_MASK_BITS(START, END)											\
			...
			// DEFINE THE BASIS FOR ALL SUPPOSED MASK RANGES
			// THE SCALE OF WHICH VARIES BASED ON OPERAND TYPES
			// AND THE AMOUNT OF DEFS

#define		M68K_MASK_RANGE_2(S1, E1, S2, E2)									\
			(M68K_MASK_BITS(S1, E1) | M68K_MASK_BITS(S2, E2))


#define		M68K_SHIFT_COUNT_IMM(IR)			M68K_EXTRACT_BITS(IR, 9, 11)
#define		M68K_SHIFT_COUNT_RY(IR)				M68K_EXTRACT_BITS(IR, 0, 2)
#define		M68K_QUICK_DATA(IR)					((M68K_EXTRACT_BITS(IR, 9, 11) == 0) ? 8 : M68K_EXTRACT_BITS(IR, 9, 11))

...
static U32 MEMORY_READ(U32 ADDRESS, U32 SIZE)
{
    ...
    {
        MEM_ERROR("[ERROR]", MEM_ERR_BOUNDS, SIZE, "ATTEMPT TO READ FROM AN ADDRESS RANGE BEYOND THE ADDRESSABLE SPACE: 0x%08X", ADDRESS);
        MEM_ERROR("[ERROR]", MEM_ERR_RESERVED, SIZE, "ATTEMPT TO READ FROM RESERVED ADDRESS RANGE: 0x%08X", ADDRESS);
        goto MALFORMED_READ;
    }
    ...
    if(MEM_BASE != NULL)
    {
        ...
        if((OFFSET + BYTES + 1) > MEM_BASE->SIZE)
        {
            ...
            MEM_ERROR("[ERROR]", MEM_ERR_BOUNDS, SIZE, "READ OUT OF BOUNDS: OFFSET = %d, SIZE = %d, VIOLATION #%u", OFFSET, BYTES, MEM_BASE->USAGE.VIOLATION);
            goto MALFORMED_READ;
        }
...
MALFORMED_READ:
    MEM_ERROR("[ERROR]", MEM_ERR_UNMAPPED, SIZE, "NO BUFFER FOUND FOR ADDRESS: 0x%08X", ADDRESS);
    MEM_ERROR("[ERROR]", MEM_ERR_BAD_READ, SIZE, "ADDRESS: 0x%08X", ADDRESS);
    return 0;
}
...
static void MEMORY_WRITE(U32 ADDRESS, U32 SIZE, U32 VALUE)
{
    ...
    if(ADDRESS > M68K_MAX_ADDR_END || ADDRESS > M68K_MAX_MEMORY_SIZE)
    {
        MEM_ERROR("[ERROR]", MEM_ERR_BOUNDS, SIZE, "ATTEMPT TO WRITE TO AN ADDRESS RANGE BEYOND THE ADDRESSABLE SPACE: 0x%08X", ADDRESS);
        MEM_ERROR("[ERROR]", MEM_ERR_RESERVED, SIZE, "ATTEMPT TO WRITE TO RESERVED ADDRESS RANGE: 0x%08X", ADDRESS);
    ...
    if(MEM_BASE != NULL)
    {
        ...
        {
            MEM_BASE->USAGE.VIOLATION++;
            MEM_ERROR("[ERROR]", MEM_ERR_READONLY, SIZE, "WRITE ATTEMPT TO READ-ONLY MEMORY AT 0x%08X, VIOLATION #%u", ADDRESS, MEM_BASE->USAGE.VIOLATION);
            goto MALFORMED_WRITE;
        }
        ...
        {
            MEM_BASE->USAGE.VIOLATION++;
            MEM_ERROR("[ERROR]", MEM_ERR_BOUNDS, SIZE, "WRITE OUT OF BOUNDS: OFFSET = %d, SIZE = %d, VIOLATION #%u", OFFSET, BYTES, MEM_BASE->USAGE.VIOLATION);
            goto MALFORMED_WRITE;
        }
...
MALFORMED_WRITE:
    MEM_ERROR("[ERROR]", MEM_ERR_BAD_WRITE, SIZE, "VALUE: 0x%0X, ADDRESS: 0x%08X", VALUE, ADDRESS);
}
...
void MEMORY_MAP(U32 BASE, U32 END, bool WRITABLE) 
{
    ...
    {
        MEM_ERROR("[ERROR]", MEM_ERR_BUFFER, SIZE, "CANNOT MAP - TOO MANY BUFFERS (%d%s)", FORMAT_SIZE(SIZE), FORMAT_UNIT(SIZE));
        return;
    }

    if(END > M68K_MAX_ADDR_END)
    {
        MEM_ERROR("[ERROR]", MEM_ERR_BUS, (U32)SIZE, "END ADDRESS 0x%08X EXCEEDS THE BUS LIMIT: (0x%08X - %d%s)", 

...
static M68K_MEM_BUFFER* MEM_FIND(U32 ADDRESS)
{
    ...
    for(unsigned INDEX = 0; INDEX < MEM_NUM_BUFFERS; INDEX++)
    {
        ...
        if(MEM_BASE->BASE != ADDRESS) { VERBOSE_TRACE("NO BUFFER FOUND FOR ADDRESS: 0x%0x\n", ADDRESS); }
        ...
        {
            VERBOSE_TRACE("ACCESSED: 0x%04X [%s] IN BUFFER %u: 0x%08X - 0x%0X\n", 
                ADDRESS, 
...
static U32 MEMORY_READ(U32 ADDRESS, U32 SIZE)
{
    ...
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_RESERVED, SIZE, "ATTEMPT TO READ FROM RESERVED ADDRESS RANGE: 0x%0X", ADDRESS);
        goto MALFORMED_READ;
    }

    if(ADDRESS > M68K_MAX_MEMORY_SIZE)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BOUNDS, SIZE, "ATTEMPT TO READ FROM AN ADDRESS RANGE BEYOND THE ADDRESSABLE SPACE: 0x%0X", ADDRESS);
        goto MALFORMED_READ;
    }
...
MALFORMED_READ:
    MEM_ERROR(MEM_ERR, MEM_ERR_BAD_READ, SIZE, "ADDRESS: 0x%0X", ADDRESS);
...
static void MEMORY_WRITE(U32 ADDRESS, U32 SIZE, U32 VALUE)
{
    ...
    VERBOSE_TRACE("WRITING TO ADDRESS 0x%0X (SIZE = %d, VALUE = 0x%0X)\n", ADDRESS, SIZE, VALUE);

    // BOUND CHECKS FOR INVALID ADDRESSING
    if(ADDRESS > M68K_MAX_ADDR_END)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_RESERVED, SIZE, "ATTEMPT TO WRITE TO RESERVED ADDRESS RANGE: 0x%0X", ADDRESS);
        goto MALFORMED_WRITE;
    }

    if(ADDRESS > M68K_MAX_MEMORY_SIZE)
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BOUNDS, SIZE, "ATTEMPT TO WRITE TO AN ADDRESS RANGE BEYOND THE ADDRESSABLE SPACE: 0x%0X", ADDRESS);
        goto MALFORMED_WRITE;
    }
...
void MEMORY_MAP(U32 BASE, U32 END, bool WRITABLE) 
{
    ...
    {
        MEM_ERROR(MEM_ERR, MEM_ERR_BUFER, SIZE, "CANNOT MAP - TOO MANY BUFFERS (%d%s)", FORMAT_SIZE(SIZE), FORMAT_UNIT(SIZE));
        return;
    }

    if(END >= M68K_MAX_MEMORY_SIZE)
    {
        // SUPPOSE THAT IF IT DOES EXCEED LIMIT AND BY PROXY ISNT DEFINED
        // IT SHOULDN'T *REALLY* HAVE A SIZE?
        MEM_ERROR(MEM_ERR, MEM_ERR_BUS, SIZE, "END ADDRESS 0x%08X EXCEEDS THE BUS LIMIT: (%d%s)", M68K_MAX_ADDR_END, FORMAT_SIZE(M68K_MAX_ADDR_END), FORMAT_UNIT(M68K_MAX_ADDR_END));
        return;
    }

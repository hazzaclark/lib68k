...
static EXCEPTION_HANDLER M68K_EXCEPTION_HANDLER_TABLE[256];
...
M68K_EXCEPTION(M68K_1111)
{
    M68K_JUMP_VECTOR(M68K_EXCEPTION_1111);

    M68K_USE_CYCLES(M68K_CYCLE[M68K_EXEC_VECTOR_TABLE[0][11]]);
    M68K_GET_CYCLES() = M68K_CYC_EXCE[M68K_REG_IR];
}
...
// PUSHES THE CURRENT SR VALUE TO THE STACK
// BASED ON THE CURRENT MODE
//
// SETS THE REQUIRED MODE NEEDED PER THOSE TICK CYCLES

M68K_EXCEPTION(PRIV_VIO)
{
    unsigned EXCEPTION_PC = M68K_REG_PC + 4;
    
    M68K_REG_SP = M68K_MASK_OUT_ABOVE_32(M68K_REG_SP - 2);
    M68K_WRITE_16(M68K_REG_SP, M68K_REG_SR);
    
    M68K_REG_SP = M68K_MASK_OUT_ABOVE_32(M68K_REG_SP - 4);
    M68K_WRITE_32(M68K_REG_SP, EXCEPTION_PC);
    
    // PUSH VECTOR FORMAT AND WRITE ACCORDINGLY
    M68K_REG_SP = M68K_MASK_OUT_ABOVE_32(M68K_REG_SP - 2);
    M68K_WRITE_16(M68K_REG_SP, 0x0008);
...
M68K_MAKE_OPCODE(ADD, 8, EA, DN)
{
    ...
    M68K_FLAG_X = M68K_FLAG_C = (RESULT > 0xFF);
    M68K_FLAG_Z = (RESULT == 0);

    M68K_DATA_HIGH = M68K_MASK_OUT_ABOVE_8(RESULT);
...
M68K_MAKE_OPCODE(ADD, 16, EA, DN)
{
    ...
    M68K_DATA_HIGH = M68K_MASK_OUT_ABOVE_16(RESULT);
    M68K_EA_PRINT_HOOK(M68K_REG_BASE);
    M68K_BASE_ADDRESS_HOOK(M68K_REG_D);
...
M68K_MAKE_OPCODE(ADD, 32, EA, DN)
{
    ...
    M68K_DATA_HIGH = M68K_MASK_OUT_ABOVE_32(RESULT);
    M68K_EA_PRINT_HOOK(M68K_REG_BASE);
    M68K_BASE_ADDRESS_HOOK(M68K_REG_D);
...
{
    unsigned* EA = &M68K_DATA_HIGH;
    unsigned SRC = M68K_MASK_OUT_ABOVE_8(M68K_DATA_LOW);
    unsigned DEST = M68K_MASK_OUT_ABOVE_8(*EA);
    unsigned RESULT = SRC + DEST;

    M68K_FLAG_N = M68K_BIT_SHIFT_8(RESULT);
    M68K_FLAG_V = M68K_FLAG_V_8(SRC, DEST, RESULT);
    M68K_FLAG_X = M68K_FLAG_C = M68K_BIT_SHIFT_8(RESULT);
    M68K_FLAG_Z = (M68K_MASK_OUT_ABOVE_8(RESULT) == 0);

    // MAKE SURE TO PRESERVE THE UPPER THE 24 BITS AND STORE LOWER 8
    *EA = M68K_MASK_OUT_BELOW_8(*EA) | M68K_MASK_OUT_ABOVE_8(RESULT);
    M68K_CCR_HOOK();
}
...
{
    unsigned* EA = &M68K_DATA_HIGH;
    unsigned SRC = M68K_MASK_OUT_ABOVE_16(M68K_DATA_LOW);
    unsigned DEST = M68K_MASK_OUT_ABOVE_16(*EA);
    unsigned RESULT = SRC + DEST;

    M68K_FLAG_N = M68K_BIT_SHIFT_16(RESULT);
    M68K_FLAG_V = M68K_FLAG_V_16(SRC, DEST, RESULT);
...
{
    unsigned* EA = &M68K_DATA_HIGH;
    unsigned SRC = M68K_MASK_OUT_ABOVE_32(M68K_DATA_LOW);
    unsigned DEST = M68K_MASK_OUT_ABOVE_32(*EA);
    unsigned RESULT = SRC + DEST;

    M68K_FLAG_N = M68K_BIT_SHIFT_32(RESULT);
    M68K_FLAG_V = M68K_FLAG_V_32(SRC, DEST, RESULT);
...
M68K_MAKE_OPCODE(ADD, 16, PRE_DEC, D)
{
    ...
    M68K_FLAG_V = ((SRC_VALUE ^ ~DEST_VALUE) & (SRC_VALUE ^ (S16)(RESULT)));
    M68K_FLAG_X = M68K_FLAG_C = M68K_BIT_SHIFT_16(RESULT);
...
M68K_MAKE_OPCODE(ADD, 8, D, EA_POST_INC)
{
    ...
    unsigned DESTINATION = M68K_READ_8(EA);
    unsigned RESULT = SRC + DESTINATION;

    M68K_FLAG_N = M68K_BIT_SHIFT_8(RESULT);
    M68K_FLAG_V = (SRC + DESTINATION + RESULT);
    M68K_FLAG_X = M68K_FLAG_C = (SRC + DESTINATION + RESULT);
...
M68K_MAKE_OPCODE(ADD, 32, D, EA_POST_INC)
{
    ...
    unsigned DESTINATION = M68K_READ_32(EA);
    unsigned RESULT = SRC + DESTINATION;
...
{
    int SRC = READ_IMM_32();
    int EA = M68K_MASK_OUT_ABOVE_32(READ_IMM_16());
    int DESTINATION = M68K_EA();
    int RESULT = SRC + DESTINATION;
    ...
    M68K_REG_PC += 2;               // ONLY ADVANCE PC SHOULD THE SIZE OF BITS BE GREATER THAN EA
}

M68K_MAKE_OPCODE(ADDQ, 8, D, 0)
{
    int DESTINATION = M68K_MASK_OUT_ABOVE_8(M68K_DATA_LOW);
    int SRC = M68K_QUICK_DATA(M68K_REG_IR);

    int RESULT = DESTINATION + SRC;

    M68K_FLAG_N = M68K_BIT_SHIFT_8(RESULT);
    M68K_FLAG_V = (((DESTINATION ^ RESULT) & (~(DESTINATION ^ SRC))) & 0x80) != 0;
    M68K_FLAG_X = M68K_FLAG_C = (RESULT < DESTINATION);
    M68K_FLAG_Z = (M68K_MASK_OUT_ABOVE_8(RESULT) == 0);

    M68K_DATA_LOW = M68K_MASK_OUT_BELOW_8(M68K_DATA_LOW) | M68K_MASK_OUT_ABOVE_8(RESULT);
...
{
    int DESTINATION = M68K_MASK_OUT_ABOVE_16(M68K_DATA_LOW);
    int SRC = M68K_QUICK_DATA(M68K_REG_IR);

    int RESULT = DESTINATION + SRC;

    M68K_FLAG_N = M68K_BIT_SHIFT_16(RESULT);
    M68K_FLAG_V = (((DESTINATION ^ RESULT) & (~(DESTINATION ^ SRC))) & 0x8000) != 0;
    M68K_FLAG_X = M68K_FLAG_C = (RESULT < DESTINATION);
    M68K_FLAG_Z = (M68K_MASK_OUT_ABOVE_16(RESULT) == 0);

    M68K_DATA_LOW = M68K_MASK_OUT_BELOW_16(M68K_DATA_LOW) | M68K_MASK_OUT_ABOVE_16(RESULT);
...
M68K_MAKE_OPCODE(ADDQ, 32, D, 0)
{
    ...
    M68K_FLAG_N = M68K_BIT_SHIFT_32(RESULT);
    M68K_FLAG_V = (((DESTINATION ^ RESULT) & (~(DESTINATION ^ SRC))) & 0x80000000) != 0;
    M68K_FLAG_X = M68K_FLAG_C = (RESULT < DESTINATION);
    M68K_FLAG_Z = (RESULT == 0);
...
M68K_MAKE_OPCODE(ADDX, 8, AN, AY)
{
    int DESTINATION = M68K_ADDRESS_HIGH;
    int SRC = M68K_MASK_OUT_ABOVE_8(M68K_ADDRESS_LOW);
...
M68K_MAKE_OPCODE(ADDX, 16, AN, AY)
{
    int DESTINATION = M68K_ADDRESS_HIGH;
    int SRC = M68K_MASK_OUT_ABOVE_16(M68K_ADDRESS_LOW);
...
M68K_MAKE_OPCODE(ADDX, 32, AN, AY)
{
    int DESTINATION = M68K_ADDRESS_HIGH;
    int SRC = M68K_MASK_OUT_ABOVE_32(M68K_ADDRESS_LOW);
...
{
    M68K_FLAG_Z = M68K_DATA_HIGH &= (READ_IMM_8());
    M68K_FLAG_N = M68K_BIT_SHIFT_8(M68K_FLAG_Z);
    M68K_FLAG_C = 0;
    M68K_FLAG_V = 0;
}
...
{
    M68K_FLAG_Z = M68K_DATA_HIGH &= (READ_IMM_16());
    M68K_FLAG_N = M68K_BIT_SHIFT_16(M68K_FLAG_Z);
    M68K_FLAG_C = 0;
    M68K_FLAG_V = 0;
}
...
{
    M68K_FLAG_Z = M68K_DATA_HIGH &= (READ_IMM_32());
    M68K_FLAG_N = M68K_BIT_SHIFT_32(M68K_FLAG_Z);
    M68K_FLAG_C = 0;
    M68K_FLAG_V = 0;
}
...
M68K_MAKE_OPCODE(ASL, 8, ASR, 0)
{
    ...
    *DEST = M68K_MASK_OUT_BELOW_8(*DEST) | M68K_MASK_OUT_ABOVE_8(RESULT);
...
M68K_MAKE_OPCODE(BCHG, 8, D, EA)
{
    ...
    unsigned MASK = 1 << (M68K_DATA_LOW & 7);
...
M68K_MAKE_OPCODE(BRA, LABEL, 0, 0)
{
    ...
    // 14/02/26 
    // ACCOUNT FOR VARYING PC DISPLACEMENT BASED ON SIGNEDNESS
...
M68K_MAKE_OPCODE(BTST, 8, EA, IMM)
{
    unsigned BIT = READ_IMM_8() & 7;
    M68K_FLAG_Z = READ_IMM_32() & (1 << BIT);
}
...
{
    unsigned SR = M68K_DATA_HIGH;
    unsigned BOUND = SR ^= M68K_DATA_LOW;

    M68K_WRITE_16(BOUND, SR);
    M68K_REG_PC += 2;
}
...
{
    M68K_ADDRESS_HIGH = M68K_LEA_AL_16();
    M68K_SET_SR(M68K_ADDRESS_HIGH);
    M68K_BASE_LEA_HOOK(M68K_REG_SP, M68K_ADDRESS_HIGH);
}
...
M68K_MAKE_OPCODE(LINK, 32, DA, 0)
{
    ...
    M68K_READ_32(*DEST);
    *DEST = M68K_REG_A[7];
    M68K_REG_A[7] = M68K_MASK_OUT_ABOVE_32(*DEST + (U16)M68K_READ_32(M68K_DATA_HIGH));
}
...
M68K_MAKE_OPCODE(MOVE, 8, D, 0)
{
    ...
    *DEST = M68K_MASK_OUT_BELOW_8(*DEST) | RESULT;

    M68K_FLAG_N = M68K_BIT_SHIFT_8(RESULT);
    M68K_FLAG_Z = M68K_BIT_ZEROED(RESULT);
    M68K_FLAG_V = 0;
    M68K_FLAG_C = 0;
...
{
    unsigned RESULT = M68K_MASK_OUT_ABOVE_16(M68K_DATA_LOW);
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 8, D, I)
{
    ...
    *EA = M68K_MASK_OUT_BELOW_8(*EA) | RESULT;
...
M68K_MAKE_OPCODE(MOVE, 16, D, I)
{
    ...
    *EA = M68K_MASK_OUT_BELOW_16(*EA) | RESULT;
...
M68K_MAKE_OPCODE(MOVE, 32, POST_INC, 0)
{
    ...
    *EA = M68K_MASK_OUT_BELOW_32(*EA) | RESULT;
...
M68K_MAKE_OPCODE(MOVE, 8, POST_INC, D)
{
    ...
    M68K_WRITE_8(READ_IMM_16(), VALUE);
}
...
// THIS IS TO MAKE SURE THAT IT DOESNT SKIP OVER THE EA
...
M68K_MAKE_OPCODE(MOVE, 8, IMM, POST_DEC)
{
    unsigned RESULT = READ_IMM_16();
...
{
    unsigned RESULT = READ_IMM_32();
    unsigned EA = M68K_EA_DECR_WORD_LONG();
...
{
    unsigned RESULT = M68K_READ_32(M68K_LEA_PI_32());
    unsigned* DEST = &M68K_DATA_HIGH;
...
{
    unsigned SRC = M68K_ADDRESS_LOW;    
    unsigned DEST = M68K_ADDRESS_HIGH;  

    unsigned RESULT = M68K_READ_8(SRC);
    M68K_MOVE_8(SRC, DEST, 1);
...
M68K_MAKE_OPCODE(MOVEA, 16, D, AY)
{
    M68K_DI_16();
    M68K_BASE_ADDRESS_HOOK(M68K_REG_A);
    M68K_EA_PRINT_HOOK(M68K_REG_BASE);
...
M68K_MAKE_OPCODE(MOVEA, 32, D, AY)
{
    M68K_DI_32();
    M68K_BASE_ADDRESS_HOOK(M68K_REG_A);
    M68K_EA_PRINT_HOOK(M68K_REG_BASE);
...
M68K_MAKE_OPCODE(MOVEA, 16, PC, AY)
{
    unsigned EA = M68K_PCDI();
    M68K_ADDRESS_HIGH = (S16)M68K_READ_16(EA);
}
...
M68K_MAKE_OPCODE(MOVE_CCR, 16, DA, 0)
{
    ...
    M68K_FLAG_X = M68K_BIT_4(M68K_ADDRESS_HIGH);
    M68K_FLAG_N = M68K_BIT_3(M68K_ADDRESS_HIGH);
    M68K_FLAG_Z = M68K_BIT_2(M68K_ADDRESS_HIGH);
    M68K_FLAG_V = M68K_BIT_1(M68K_ADDRESS_HIGH);
    M68K_FLAG_C = M68K_BIT_0(M68K_ADDRESS_HIGH);
...
M68K_MAKE_OPCODE(MOVEM, 16, POST_DEC, A)
{
    unsigned INDEX = 0;
    ...
    for(; INDEX < 16; INDEX--)  
    {
        ...
        {
            EA -= 2; 
            unsigned int REG_VALUE = M68K_GET_REGISTERS(INDEX); 
            M68K_WRITE_16(EA, M68K_REG_BASE[REG_VALUE]);
            COUNT++;  
        }
    }
...
M68K_MAKE_OPCODE(MOVEM, 32, POST_DEC, A)
{
    unsigned INDEX = 0;
    unsigned LIST = READ_IMM_16();
    unsigned EA = M68K_ADDRESS_LOW;
    ...
    for(; INDEX < 16; INDEX--)  
    {
        if(LIST & (1 << INDEX))  
        {
            EA -= 4; 
            unsigned int REG_VALUE = M68K_GET_REGISTERS(INDEX); 
            M68K_WRITE_32(EA, M68K_REG_BASE[REG_VALUE]);
            COUNT++;  
        }
    }
...
M68K_MAKE_OPCODE(MOVEP, 16, ER, 0)
{
    ...
    M68K_WRITE_8(EA, M68K_MASK_OUT_BELOW_8(M68K_BIT_SHIFT_N_16(SRC)));
...
{
    unsigned EA = M68K_MASK_OUT_ABOVE_32(READ_IMM_16());
    unsigned SRC = M68K_DATA_HIGH;

    M68K_WRITE_8(EA, M68K_MASK_OUT_ABOVE_8(M68K_BIT_SHIFT_N_32(SRC)));
    M68K_WRITE_8(EA += 2, M68K_MASK_OUT_ABOVE_8(M68K_BIT_SHIFT_N_24(SRC)));
    M68K_WRITE_8(EA += 2, M68K_MASK_OUT_ABOVE_8(M68K_BIT_SHIFT_N_16(SRC)));
    M68K_WRITE_8(EA += 2, M68K_MASK_OUT_ABOVE_8(SRC));
...
M68K_MAKE_OPCODE(MOVE, 8, PD, DISP)
{
    unsigned RESULT = M68K_ADDRESS_LOW + READ_IMM_16();
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 16, PD, DISP)
{
    unsigned RESULT = M68K_ADDRESS_LOW + READ_IMM_16();
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 32, PD, DISP)
{
    unsigned RESULT = M68K_ADDRESS_LOW + READ_IMM_32();
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 8, AY, IND)
{
    unsigned RESULT = M68K_DATA_LOW;
    unsigned EA = M68K_ADDRESS_HIGH;
    ...
    M68K_MOVE_8(EA, RESULT, 1);
}

M68K_MAKE_OPCODE(MOVE, 16, AY, IND)
{
    unsigned RESULT = M68K_DATA_LOW;
    unsigned EA = M68K_BIT_SHIFT_N_24(M68K_ADDRESS_HIGH);
...
M68K_MAKE_OPCODE(MOVE, 8, AN, IND)
{
    unsigned RESULT = M68K_ADDRESS_LOW;
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 16, AN, IND)
{
    unsigned RESULT = M68K_ADDRESS_LOW;
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 32, AN, IND)
{
    unsigned RESULT = M68K_ADDRESS_LOW;
    unsigned* DEST = &M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MULS, 16, D, 0)
{
    unsigned* DEST = &M68K_DATA_LOW;
    unsigned SRC = M68K_DATA_LOW;
    unsigned RESULT = M68K_MASK_OUT_ABOVE_32(SRC * (M68K_MASK_OUT_ABOVE_16(*DEST)));
...
M68K_MAKE_OPCODE(MULS, 16, EA, DY)
{
    unsigned* DEST = &M68K_DATA_HIGH;
    unsigned SRC = READ_IMM_32();
    unsigned RESULT = M68K_MASK_OUT_ABOVE_32(SRC * (U16)(M68K_MASK_OUT_ABOVE_16(*DEST)));
...
M68K_MAKE_OPCODE(MULU, 16, EA, DY)
{
    unsigned* DEST = &M68K_DATA_HIGH;
    unsigned SRC = READ_IMM_32();
    unsigned RESULT = SRC * M68K_MASK_OUT_ABOVE_16(*DEST);
...
M68K_MAKE_OPCODE(ROL, 8, S, 0)
{
    ...
    unsigned SRC = M68K_MASK_OUT_ABOVE_8(*DEST);
    unsigned RESULT = M68K_MASK_OUT_ABOVE_8(SRC);
...
M68K_MAKE_OPCODE(ROL, 16, S, 0)
{
    ...
    unsigned SRC = M68K_MASK_OUT_ABOVE_32(*DEST);
    unsigned RESULT = M68K_MASK_OUT_ABOVE_16(SRC);
...
M68K_MAKE_OPCODE(ROL, 32, S, 0)
{
    ...
    unsigned SRC = M68K_MASK_OUT_ABOVE_32(*DEST);
    unsigned RESULT = M68K_MASK_OUT_ABOVE_32(SRC);
...
M68K_MAKE_OPCODE(STOP, 0, 0, 0)
{
    ...
    // WE ONLY EVER WANT TO SET THE SR 
    // SHOULD WE BE USING SUPERVISOR MDOE

    // SR MODE ENCOMPASSES 4 BYTES TO HANDLE IMM
    // COMPARED FOR 2 BYTES FOR ABSOLUTE OPERAND
...
M68K_MAKE_OPCODE(SUB, 8, D, 0)
{
    ...
    M68K_FLAG_N = M68K_BIT_SHIFT_8(RESULT);
    M68K_FLAG_X = (M68K_FLAG_C = RESULT == 0);
    M68K_FLAG_V = (((DEST ^ SRC) & (DEST ^ RESULT)) & 0x80) != 0;
    M68K_FLAG_Z = (M68K_MASK_OUT_ABOVE_8(RESULT) == 0);
...
M68K_MAKE_OPCODE(SUB, 16, D, 0)
{
    ...
    M68K_FLAG_N = M68K_BIT_SHIFT_16(RESULT);
    M68K_FLAG_X = (M68K_FLAG_C = RESULT == 0);
    M68K_FLAG_V = (((DEST ^ SRC) & (DEST ^ RESULT)) & 0x8000) != 0;
    M68K_FLAG_Z = (M68K_MASK_OUT_ABOVE_16(RESULT) == 0);
...
M68K_MAKE_OPCODE(SUB, 32, D, 0)
{
    ...
    M68K_FLAG_N = M68K_BIT_SHIFT_32(RESULT);
    M68K_FLAG_X = (M68K_FLAG_C = RESULT == 0);
    M68K_FLAG_V = (((DEST ^ SRC) & (DEST ^ RESULT)) & 0x80000000) != 0;
    M68K_FLAG_Z = (M68K_MASK_OUT_ABOVE_32(RESULT) == 0);
...
OPCODE_HANDLER M68K_OPCODE_HANDLER_TABLE[] =
{
    ...
    {ADDA_32_AN_AY,             0xF1F8,     0xD1C8,     12},  // ADDA.L An, Ay
    ...
    {ADDX_8_AN_AY,              0xF1F8,     0xD108,     8},  // ADDX.B -(An), -(Ay)
    {ADDX_16_AN_AY,             0xF1F8,     0xD148,     8},  // ADDX.W -(An), -(Ay)
    {ADDX_32_AN_AY,             0xF1F8,     0xD188,     12},  // ADDX.L -(An), -(Ay)
    ...
    {BTST_8_EA_IMM,             0xFFFF,     0x0839,     20},  /// BTST #imm, <ea>
    ...
    {MOVE_8_D_0,                0xF1F8,     0x1000,     8},   // MOVE.B Dn,Dy 
    {MOVE_16_D_0,               0xF1F8,     0x3000,     8},   // MOVE.W Dn,Dy 
    {MOVE_32_D_0,               0xF1F8,     0x2000,     20},   // MOVE.L Dn,Dy
    {MOVE_8_A_0,                0xF1F8,     0x1008,     4},   // MOVE.B An, Ay
    {MOVE_16_A_0,               0xF1F8,     0x3008,     8},   // MOVE.W An, Ay
    {MOVE_32_A_0,               0xF1F8,     0x2008,     20},   // MOVE.L An, Ay
    ...
    {MOVEA_16_PC_AY,            0xF1FF,     0x307A,     16},  // MOVEA.W d(PC), Ay
    ...
    {MOVE_8_PD_DISP,            0xF1F8,     0x1028,     10},  // MOVE.B -d(Dx), Dy 
    {MOVE_16_PD_DISP,           0xF1F8,     0x3028,     10},  // MOVE.B -d(Dx), Dy 
    {MOVE_32_PD_DISP,           0xF1F8,     0x2028,     20},  // MOVE.L -d(Dx), Dy
    {MOVE_8_AY_IND,             0xF1F8,     0x1080,     10},  // MOVE.B Dn, (Ay)
    {MOVE_16_AY_IND,            0xF1F8,     0x3080,     10},  // MOVE.W Dn, (Ay)
    {MOVE_32_AY_IND,            0xF1F8,     0x2080,     20},  // MOVE.L Dn, (Ay)
    {MOVE_8_AN_IND,             0xF1F8,     0x1010,     20},  // MOVE.B (An), Dy
    {MOVE_16_AN_IND,            0xF1F8,     0x3010,     20},  // MOVE.W (An), Dy
    {MOVE_32_AN_IND,            0xF1F8,     0x2010,     24},  // MOVE.L (An), Dy
    ...
    {ORI_8_D_0,                 0xFFF8,     0x0000,     8},  // ORI.B #<data>,<ea>
    {ORI_16_D_0,                0xFFF8,     0x0040,     8},  // ORI.W #<data>,<ea>
    {ORI_32_D_0,                0xFFF8,     0x0080,     16}, // ORI.L #<data>,<ea>
...
void M68K_BUILD_OPCODE_TABLE(void)
{
    ...
    // DEFAULT TO ILLEGAL SHOULD WE NOT FIND A MATCH
    // IN THE MAX MEMORY BUFFER
...
// DO THE SAME FOR THE ENCOMPASSING EXCEPTION HANDLERS THAT ARE VITAL AT RUNTIME
// THE BENEFIT OF HAVING THIS IS THAT WE ARE ABLE TO DYNAMICALLY ASSERT
...
void M68K_BUILD_EXCEPTION_TABLE(void)
{
    ...
    M68K_EXCEPTION_HANDLER_TABLE[10] = (EXCEPTION_HANDLER){10, M68K_VECTOR_TABLE[0][10], M68K_1010};
    M68K_EXCEPTION_HANDLER_TABLE[11] = (EXCEPTION_HANDLER){11, M68K_VECTOR_TABLE[0][11], M68K_1111};
    M68K_EXCEPTION_HANDLER_TABLE[4] = (EXCEPTION_HANDLER){4, M68K_VECTOR_TABLE[0][4], ILLEGAL};

...
{
    M68K_BRANCH_8(M68K_REG_PC);
    return;
}

M68K_MAKE_OPCODE(BEQ, 16, 0, 0)
{
    M68K_BRANCH_16(M68K_REG_PC);
    return;
}

M68K_MAKE_OPCODE(BEQ, 32, 0, 0)
{
    M68K_BRANCH_32(M68K_REG_PC);
    return;
}
...
M68K_MAKE_OPCODE(MOVE, 16, IMM, D)
{
    unsigned RESULT = M68K_DATA_HIGH;
...
M68K_MAKE_OPCODE(MOVE, 16, D, POST_INC)
{
    ...
    M68K_WRITE_16(M68K_DATA_HIGH, VALUE);
}
...
{
    unsigned RESULT = M68K_POST_DEC_16();
    unsigned* DEST = &M68K_DATA_LOW;
...
M68K_MAKE_OPCODE(NEG, 8, D, 0)
{
    ...
    M68K_FLAG_C = M68K_FLAG_X = READ_IMM_8();
    M68K_FLAG_V = *DEST & RESULT;
    M68K_FLAG_Z = M68K_MASK_OUT_ABOVE_8(RESULT);

    *DEST = *DEST | M68K_FLAG_Z;

    M68K_WRITE_8(RESULT, M68K_FLAG_Z);
}
...
M68K_MAKE_OPCODE(NEG, 16, D, 0)
{
    ...
    M68K_FLAG_C = M68K_FLAG_X = READ_IMM_16();
    M68K_FLAG_V = *DEST & RESULT;
    M68K_FLAG_Z = M68K_MASK_OUT_ABOVE_16(RESULT);

    *DEST = *DEST | M68K_FLAG_Z;

    M68K_WRITE_16(RESULT, M68K_FLAG_Z);
}
...
M68K_MAKE_OPCODE(NEG, 32, D, 0)
{
    ...
    M68K_FLAG_C = M68K_FLAG_X = READ_IMM_32();
    M68K_FLAG_V = *DEST & RESULT;
    M68K_FLAG_Z = M68K_MASK_OUT_ABOVE_32(RESULT);
...
OPCODE_HANDLER M68K_OPCODE_HANDLER_TABLE[] =
{
    ...
    {ADDQ_32_I_0,               0xF1F8,     0x50A8,     24}, // ADDQ.B #imm, Dn
    ...
    {MOVE_32_IMM_D,             0xF1F8,     0x203C,     16},  // MOVE.L #<data>,Dn 
    ...
    {MOVE_16_D_POST_INC,        0xF1F8,     0x3018,     8},   // MOVE.W (An)+,Dn
    {MOVE_16_D_POST_DEC,        0xF1F8,     0x3F00,     8},   // MOVE.W Dn, (-SP)
    {MOVE_32_POST_INC_0,        0xF1F8,     0x2018,     12},  // MOVE.L (An)+,Dn

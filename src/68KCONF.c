...
void INITIALIZE_MEMORY(void)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        CPU.MEMORY_MAP[i].MEMORY_BASE = MAX_MEMORY_BUFFER;
        CPU.MEMORY_MAP[i].MEMORY_READ_8 = &M68K_READ_8;
        CPU.MEMORY_MAP[i].MEMORY_READ_16 = &M68K_READ_16;
        CPU.MEMORY_MAP[i].MEMORY_READ_32 = &M68K_READ_32;
    }
}
...
U32 M68K_READ_32(U32 ADDRESS)
{
    ...
    if (CPU.MEMORY_MAP[BANK].MEMORY_BASE != NULL) 
	{
        return ((U32)CPU.MEMORY_MAP[BANK].MEMORY_BASE[OFFSET] << 24) | 
               ((U32)CPU.MEMORY_MAP[BANK].MEMORY_BASE[OFFSET + 1] << 16) |
               ((U32)CPU.MEMORY_MAP[BANK].MEMORY_BASE[OFFSET + 2] << 8) |
               ((U32)CPU.MEMORY_MAP[BANK].MEMORY_BASE[OFFSET + 3]);
    }
...
unsigned int READ_IMM_16(void)
{
	...
	// USE THE TILDA TO NEGATE THE OUTCOME OF THE ABOVE MASK
	// THIS MEANS THAT IT WILL DO THE INVERSE

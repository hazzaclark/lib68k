...
void M68K_PULSE_RESET(void)
{
	...
	// FIX - READ FROM ACTUALLY VECTOR TABLE VALUES RATHER THAN PREFETCH
    M68K_REG_SP = M68K_READ_32(0);
    M68K_REG_PC = M68K_READ_32(4);
...
// HELPER FUNCTION BE ABLE TO EXTRAPOLATE THE CURRENT
// PC LOCATION IN RELATION TO A GENERIC EFFECTIVE ADDRESS
// RETURNS A MANUAL PC ADVANCE AGAINST THE EA
unsigned int M68K_PCDI(void)
{
	unsigned PREVIOUS = M68K_REG_PC;
...
int LOAD_BINARY_FILE(const char* FILE_PATH, U32 LOAD_ADDR)
{
    ...
    // LEVERAGE MEM FIND TO FIND THE CURRENT
	// ADDRESS WITHIN MEMORY TO LOAD DIRECTLY INTO THE BUFFER
    M68K_MEM_BUFFER* MEM_BASE = MEM_FIND(LOAD_ADDR);
    
    if(!MEM_BASE || !MEM_BASE->BUFFER)
    {
        printf("ERROR: NO MEMORY BUFFER FOUND AT ADDRESS 0x%08X\n", LOAD_ADDR);
        free(BUFFER);
        return -1;
    }

    // DIRECTLY COPY TO THE MEMORY BUFFER
	// THIS HELPS WHEN BEING ABLE TO PROPERLY LOAD ADJACENT
	// OF A FIXED SIZE BUFFER
    U32 OFFSET = LOAD_ADDR - MEM_BASE->BASE;
    
    long COPY_BYTES = FILE_SIZE;
    if(OFFSET + FILE_SIZE > MEM_BASE->SIZE)
    {
        COPY_BYTES = MEM_BASE->SIZE - OFFSET;
        printf("WARNING: FILE TRUNCATED TO FIT IN MEMORY BUFFER\n");
    }
    
	// COPY THAT NOW INTO MEMORY AGAINST ALL CORRESPONDENCE
    memcpy(MEM_BASE->BUFFER + OFFSET, BUFFER, COPY_BYTES);    
	
	// ADDED THE UPDATE TO THE USAGE PARAMETERS BECAUSE
	// LOADING INTO THE BUFFER FROM MEM_FIND DOESN'T NECESSITATE
	// THE MEANS OF LOADING AGAINST THE USAGE PARAMETERS
	MEM_BASE->USAGE.WRITE_COUNT += COPY_BYTES;
	MEM_BASE->USAGE.ACCESSED = true;
	MEM_BASE->USAGE.LAST_WRITE = LOAD_ADDR + COPY_BYTES - 1;

    printf("LOADED %ld BYTES DIRECTLY INTO MEMORY BUFFER\n", COPY_BYTES);

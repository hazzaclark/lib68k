...
// GET THE EXTENSION OF A PRESUPPOSED IMMEDIATE VALUE
// AND OR EA FROM AN OPCODE

// THE IDEA IS THAT WE LOOK WITHIN 13 BITS OF THE 
...
int LOAD_BINARY_FILE(const char* FILE_PATH, U32 LOAD_ADDR)
{
    ...
    long BYTES_READ = fread(BUFFER, 1, FILE_SIZE, FILE_PTR);
    fclose(FILE_PTR);

    if(BYTES_READ != (long)FILE_SIZE)
    {
        printf("ERROR: FILE READ INCOMPLETE (%ld/%ld BYTES)\n", BYTES_READ, FILE_SIZE);
        free(BUFFER);
        return -1;
    }

    for(long i = 0; i < FILE_SIZE; i++) { M68K_WRITE_8(LOAD_ADDR + i, BUFFER[i]); }

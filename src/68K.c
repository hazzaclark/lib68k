...
{
    M68K_REG_PC = NEW_PC;
    // Prefetch the instruction at the new PC
...
void M68K_JUMP_VECTOR(unsigned VECTOR)
{
    ...
    // PREFETCH THE NEW INSTRUCTION AFTER THE NEW PC
    M68K_REG_IR = M68K_READ_16(M68K_REG_PC);
    M68K_REG_PC += 2;
}

/* PERFORM A LOGICAL BITWISE AND FROM THE SOURCE DESTINATION */
...
void M68K_SET_SR(unsigned VALUE)
{
    ...
    M68K_SET_CCR(VALUE);
    M68K_SET_S_FLAG(M68K_BIT_D(VALUE));
    M68K_SET_SR_IRQ(VALUE);
}
...
{   
    while(!M68K_CPU_STOPPED)
    {
        ...
        //
        // THE REASON FOR DOING AN IMMEDIATE READ IS 
        // DUE TO THE WAY IN WHICH THE PC FETCHES THE WORD AND ADVANCES
